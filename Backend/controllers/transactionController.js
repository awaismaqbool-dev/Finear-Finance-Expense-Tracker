...
if (source === "cash") {
    ...
    await user.save();
    await TransactionModel.findOneAndDelete({ _id: transactionId, userId: userId });
    res.status(200).json({ 
...
export const withdrawSavings = async (req, res) => {
    ...
    try {
        ...
        if (profile.flexibleSavings < amount) {
            return res.status(400).json({ success: false, message: "savings has not enough money" });
        }

        // 2. savings se minus karo aur Target (Hand/Bank) mein plus karo
        const balanceField = targetSource === 'Bank' ? 'bankBalance' : 'handBalance';
...
export const updateTrasnaction = async (req, res) => {
    const { transactionId, title,category } = req.body;
    const userId = req.userId || req.body.id;
    ...
    try {
        ...
        if (!updatedtransaction) return res.status(404).json({ success: false, message: "transaction not fonud" });
...
export const exportTransactionsToExcel = async (req, res) => {
    try {
        ...
        // Dynamic File Name
        const fileLabel = type ? type.charAt(0).toUpperCase() + type.slice(1) : "All";
